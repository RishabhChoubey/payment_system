...
public class KafkaEventProducer {
    ...
    public void sendTransactionEvent(String key, Transaction transaction) {
        logger.info("[KAFKA-SEND] Sending to Kafka - Topic: {}, Key: {}, Message: {}", TOPIC, key, transaction);

        CompletableFuture<SendResult<String, Transaction>> future = kafkaTemplate.send(TOPIC, key, transaction);

        future.thenAccept(result -> {
            RecordMetadata metadata = result.getRecordMetadata();
            logger.info("[KAFKA-SUCCESS] Message sent - Topic: {}, Partition: {}, Offset: {}",
                metadata.topic(), metadata.partition(), metadata.offset());
        }).exceptionally(ex -> {
            logger.error("[KAFKA-ERROR] Failed to send message - Key: {}, Error: {}", key, ex.getMessage(), ex);
            return null;
        });
    }
}
